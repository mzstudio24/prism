  ...
  const plans = [
    {
      ...
      features:[
        ...
        { text: "Citation Generator", ok: false }, { text: "Writing Authenticity Analyzer", ok: false }, { text: "Unlimited usage", ok: false },
      ],
    },
    {
      ...
      features:[
        ...
        { text: "Citation Generator", ok: true }, { text: "Early access to new features", ok: true }, { text: "Priority support via email", ok: true },
      ],
    },
    {
      ...
      features:[
        { text: "Everything in Pro", ok: true }, { text: "Writing Authenticity Analyzer", ok: true }, { text: "Deeper analysis reports", ok: true }, { text: "First access to every new feature", ok: true }, { text: "Priority support via email", ok: true }, { text: "Direct feedback line to founder", ok: true },
      ],
    },
  ];
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(6,8,16,0.95)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:50,padding:20}} onClick={onClose}>
      <div style={{background:"#0b0f1a",border:"1px solid rgba(99,179,237,0.15)",padding:"32px 28px",maxWidth:630,width:"100%",maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(160px, 1fr))",gap:10,marginBottom:20,alignItems:"start"}}>
          {plans.map(plan=>(
            <div key={plan.key} style={{border:`1px solid ${plan.highlight?"rgba(99,179,237,0.55)":"rgba(255,255,255,0.06)"}`,background:plan.highlight?"rgba(99,179,237,0.05)":"transparent",position:"relative",transform:plan.highlight?"scale(1.04)":"scale(1)",boxShadow:plan.highlight?"0 0 28px rgba(99,179,237,0.07)":"none"}}>
              ...
              <div style={{padding:"22px 14px 18px"}}>
                ...
                <button disabled={plan.ctaOff}onClick={()=>{
  if(plan.key==="pro") window.open("https://buy.stripe.com/test_4gMfZh3Eu1U66Wnd0I8AE00","_blank");
  if(plan.key==="scholar") window.open("https://buy.stripe.com/test_8x2cN57UK7eq3Kb4uc8AE01","_blank");
...
// ─── ANALYZE TAB ──────────────────────────────────────────────
function AnalyzeTab({ tier, usedAnalyze, setUsedAnalyze, onUpgrade, user, onLogin }) {
...
// ─── SUMMARIZE TAB ────────────────────────────────────────────
function SummarizeTab({ tier, usedSummarize, setUsedSummarize, onUpgrade, user, onLogin }) {
  const [text,setText]=useState("");
...
// ─── CHAT TAB ─────────────────────────────────────────────────
function ChatTab({ tier, user, onLogin }) {
...
// ─── CITATION TAB ─────────────────────────────────────────────
function CitationTab({ tier, onUpgrade, user, onLogin }) {
  const [text,setText]=useState("");
...
// ─── PLAGIARISM TAB ───────────────────────────────────────────
function PlagiarismTab({ tier, onUpgrade, user, onLogin }) {
  const [text,setText]=useState("");
  const [result,setResult]=useState(null);
  const [loading,setLoading]=useState(false);
...
export default function PrismApp() {
  ...
  if (authLoading) return <div style={{background:"#060810",height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",color:"#63b3ed",fontFamily:"'JetBrains Mono',monospace",fontSize:12,letterSpacing:3}}>LOADING...</div>;
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .content{position:relative;z-index:1;max-width:760px;margin:0 auto;padding:0 16px 80px;}
        ...
        .login-btn{padding:7px 16px;background:#63b3ed;border:none;color:#060810;font-family:'Syne',sans-serif;font-size:10px;font-weight:800;letter-spacing:2px;cursor:pointer;transition:all 0.2s;}
        .logout-btn{padding:7px 16px;background:transparent;border:1px solid rgba(255,255,255,0.1);color:#4a5568;font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:2px;cursor:pointer;}
      ...
      <div className="app">
        ...
        <div className="content">
          <div className="header">
            ...
            <div className="logo-sub">REFRACT. REFLECT. IMPROVE.</div>
          </div>
          <div className="topbar">
            <div className="status">
              <div className="dot"/>
              SYSTEM ONLINE · <span style={{color:tier==="free"?"#4a5568":tier==="pro"?"#63b3ed":"#b794f4",marginLeft:4}}>{tier.toUpperCase()}</span>
              {user && <span style={{marginLeft:8,color:"#2a3040"}}>· {user.displayName}</span>}
            </div>
          ...
          {activeTab==="analyze" && <AnalyzeTab tier={tier} usedAnalyze={usedAnalyze} setUsedAnalyze={handleSetUsedAnalyze} onUpgrade={openUpgrade} user={user} onLogin={handleLogin}/>}
          {activeTab==="summarize" && <SummarizeTab tier={tier} usedSummarize={usedSummarize} setUsedSummarize={handleSetUsedSummarize} onUpgrade={openUpgrade} user={user} onLogin={handleLogin}/>}
          {activeTab==="chat" && <ChatTab tier={tier} user={user} onLogin={handleLogin}/>}
          {activeTab==="citation" && <CitationTab tier={tier} onUpgrade={openUpgrade} user={user} onLogin={handleLogin}/>}
          {activeTab==="plagiarism" && <PlagiarismTab tier={tier} onUpgrade={openUpgrade} user={user} onLogin={handleLogin}/>}
        </div>
      </div>
    </>
  );
